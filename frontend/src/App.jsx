...
// ── Login Page ────────────────────────────────────────────────────────────────
const LoginPage = ({ onLogin, onGoogleLogin, modal = false }) => {
    ...
    try {
      ...
      const handleMessage = async (event) => {
        ...
        if (event.data === "google-auth-success") {
          ...
          popup.close();
          await onGoogleLogin();
        } else if (event.data === "google-auth-error:domain-not-allowed") {
          window.removeEventListener("message", handleMessage);
          setError("Only users from the allowed domain can sign in with Google.");
        }
      };
    ...
    <div className={modal ? "w-full max-w-sm" : "min-h-screen bg-[#F8FAFC] flex items-center justify-center p-4"}>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-8">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-5 py-3 bg-white border border-slate-200 text-slate-700 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-50 transition-all cursor-pointer"
          >
              ...
              <>
                ...
                <div className="relative" ref={inviteRef}>
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer border ${
                      inviteUrl
                  ...
                  {inviteUrl && (
                    <div className="absolute right-0 top-full mt-2 z-[200] bg-white border border-slate-200 shadow-2xl rounded-2xl p-4 w-80" onMouseDown={(e) => e.stopPropagation()}>
                      <div className="text-[10px] font-black text-slate-400 uppercase mb-2">Invite Link (valid 1 hour)</div>
                      <div className="flex items-center gap-2">
                        <input
                          readOnly
                          value={inviteUrl}
                          className="flex-1 text-[11px] bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-slate-600 font-mono truncate"
                        />
                        <button
                          onClick={() => { navigator.clipboard.writeText(inviteUrl); setInviteCopied(true); setTimeout(() => setInviteCopied(false), 2000); }}
                          className="flex items-center gap-1 px-2 py-1.5 bg-slate-900 text-white rounded-lg text-[10px] font-black cursor-pointer hover:bg-black transition-all flex-shrink-0"
                        >
                          <Copy size={11} /> {inviteCopied ? "Copied!" : "Copy"}
                        </button>
                      </div>
                    </div>
                  )}
