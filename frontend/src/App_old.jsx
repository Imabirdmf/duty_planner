...
const App = () => {
  ...
  const [activePopover, setActivePopover] = useState(null); // ID пользователя, для которого открыт календарь

  // --- Вспомогательные функции ---

  // Обновленный трансформер под новую структуру ответа
  const transformDuties = (data) => {
  ...
  // Добавление выходного с обработкой специфических ошибок бэкенда
  ...
  // Метод генерации с новым Payload
  const handleGenerate = async () => {
    ...
    try {
      const res = await api.post('/api/duties/generate/', {
        ...
        dates: selectedDutyDays // Теперь даты передаются здесь
      });
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-2xl flex items-center justify-between animate-in fade-in zoom-in sticky top-4 z-50 shadow-lg">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-100 gap-4">
          ...
          {/* Кнопка "Сохранить календарь" удалена */}
        </header>
